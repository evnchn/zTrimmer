...
def press(event):
    ...
    if event.key in ".":
        ...
        if not confirm_key == str(event.key):
            ...
            plt.title("[Enter]: clipboard, [+]: browser, [-]: file")
            plt.draw()
            return
        else:
            output = BytesIO()
            ...
            sys.exit()
    elif event.key in "+" and confirm_key == "enter":
        try:
            os.mkdir("output")
        except:
            pass
        ...
        sys.exit()
    confirm_key = None
    plt.title("[Enter]: save. [.]: quit. [-]: reset.")
...
fig, ax = plt.subplots(num='[Enter]: save. [.]: quit. [-]: reset.')
fig.canvas.mpl_connect("key_press_event",press)
imgplot = plt.imshow(img)
plt.title("[Enter]: save. [.]: quit. [-]: reset.")
plt.show()